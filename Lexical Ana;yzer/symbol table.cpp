...
bool SymbolTable::Remove(char* name){
    int off=0,pos=0;
    if(!currentScope->searchItem2(name,off,pos)){
            printf("Not Found!\n");
            return false;
    }
    else{
        listNode* l=currentScope->searchItem2(name,off,pos);
        printf("Found in ScopeTable# %d at position %d, %d\n",currentScope->getId(),pos,off);
        return currentScope->deleteItem(name);
    }
}
listNode* SymbolTable::Lookup(char* name)
{
    ScopeTable* temp=currentScope;
    int off=0,pos=0;
    while(temp)
    {
        if(!temp->searchItem2(name,off,pos) && temp->getParent()){
            //printf("I m here %p   %d ",temp->getParent(),temp->getId());
            temp=temp->getParent();
        }
        else{
            listNode* l=temp->searchItem2(name,off,pos);
            printf("Found in ScopeTable# %d at position %d, %d\n",temp->getId(),pos,off);
            return l;
        }
    }
