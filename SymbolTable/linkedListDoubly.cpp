...
public:
    ...
    int insertLast(SymbolInfo s) ;
    int deleteItem(char* value);
    listNode* searchItem(char* key,int& off);
    listNode* searchItem2(char* key,int& off,char* type);
...
int linkList::insertLast(SymbolInfo s) 
{
    ...
    if(list==0) //inserting the first item
    {
        ...
        newNode->prev = 0;
        list = newNode;
        //printf("dd<%s> %s\n",list->item.getType(),list->item.getName());
        return off;
    }
    else
    {
...
listNode* linkList::searchItem2(char* key,int& off,char* type)
{
    struct listNode * temp ;
    off=0;
    temp = list ; //start at the beginning
    while (temp != 0)
    {
        if (!strcmp(temp->item.getName(),key) && !strcmp(temp->item.getIDType(),type))
	{
            return temp ;
	}
        temp = temp->next ; //move to next node
...
void linkList::printListInFile(FILE* output)
{
    ...
    while(temp!=0)
    {
        fprintf(output,"<%s,%s>  ", temp->item.getName(),temp->item.getIDType());
        temp = temp->next;
    }
    fprintf(output,"\n");
}
