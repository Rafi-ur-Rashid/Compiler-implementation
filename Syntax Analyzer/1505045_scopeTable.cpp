...
public:
    ...
    void clear();
    int getLength();
    listNode* searchItem(char* key,int& x,int& p);
    listNode* searchItem2(char* key,int& off,int& p,char* type);
...
listNode* ScopeTable::searchItem(char* key,int& x,int& p)
{
    int i=0,off;
    int pos;
...
listNode* ScopeTable::searchItem2(char* key,int& x,int& p,char* type)
{
    int i=0,off;
    int pos;
        pos=hash3(key);
 //cout<<pos<<endl;
    listNode* temp= T[pos].searchItem2(key,off,type);
    if(temp){
        x=off;
        p=pos;
    }
...
void ScopeTable::printListInFile(FILE* output)
{
    ...
    if(length==0 )
    {
        fprintf(output,"The Table is empty\n");       
	return;
    }
    for(i=0; i<Tlength; i++){
        if(!T[i].checkEmpty()){
